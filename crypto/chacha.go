...
// Chacha is an instance of the ChaCha stream cipher. It implements both
...
var _ cipher.Stream = (*Chacha)(nil)
var _ io.Reader = (*Chacha)(nil)

// NewChacha returns an initialized instance of a new ChaCha cipher. A ChaCha
// key is 32 bytes and a ChaCha IV is 8 bytes, so len(key) must be >= 32
// and len(iv) must be >= 8. Rounds should be one of 8, 12, or 20.
func NewChacha(key, iv []byte, rounds int) *Chacha {
	c := new(Chacha)
